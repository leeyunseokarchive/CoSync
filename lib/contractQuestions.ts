...
export type QuestionInfo = {
  ...
  /** 항상 펼쳐 보여주는 안내. "여기서 정하지 말고 전문가에게 가라" 같은 것.
   *  2차 변호사 자문에서 같은 형태의 지시가 여러 문항에 나온다. */
  advisory?: string;
};
...
export const CONSENT_ITEMS: { plain: string; text: string }[] = [
  ...
  { plain: "1억 원을 넘는 돈을 다른 곳에 투자하기", text: "회사가 1억 원을 초과하여 투자하는 행위" },
];
...
export const CONTRACT_QUESTIONS: ContractQuestion[] = [
  ...
  },
  {
    ...
    topic: "투자 유치 기준 금액",
    consensus: true,
    // 2차 자문 4-4: "모든 투자는 같이 의논하되, 10억 미만인 경우에는 대표자 혼자 투자를
    // 받을 수 있고, 10억 이상인 경우에는 같이 의사를 모으고 ... 합치가 안 되는 경우에는
    // 대표자가 단독으로 결정." 가운데와 끝은 2호·③항이 이미 한다. 앞의 선만 여기서 정한다.
    title: "얼마 이상 투자를 받을 때 다 같이 정하나요?",
    desc: "이 금액보다 작은 투자는 대표 주주가 혼자 받을 수 있습니다. 이상이면 주주 전원이 함께 정하고, 그래도 합의가 안 되면 제2조 ③항에 따라 대표 주주가 정합니다.",
    template: {
      type: "amount",
      // 변호사가 든 10억은 시리즈 A급 예시다. 이 계약서를 쓰는 팀은 대개 그 앞 단계라
      // 10억이 최저 선이면 사실상 모든 라운드가 대표 단독이 된다. 아래쪽을 열어 둔다.
      presets: [
    ...
    preview: {
      ...
      blocks: [
        ...
        ...CONSENT_ITEMS.map((c, i) => ({
          kind: "para" as const,
          indent: true,
          // 2호에만 단서를 붙인다. 나머지 호는 원문 그대로다.
          text: i === 1
            ? `2. ${c.text} 다만 1회 {0} 미만의 투자 유치는 대표 주주가 단독으로 결정할 수 있다.`
            : `${i + 1}. ${c.text}`,
        })),
        { kind: "ellipsis" },
      ],
    ...
    info: {
      what: "제2조 ①항 2호의 주식·사채 발행이 곧 투자 유치입니다. 원문은 금액과 상관없이 주주 전원의 합의를 요구해서, 작은 투자도 한 명이 반대하면 받을 수 없습니다. 여기서 정한 금액 아래로는 대표 주주가 혼자 받을 수 있게 선을 긋습니다.",
      ifUnset: "선이 없으면 모든 투자 유치가 전원 합의 대상이 됩니다. 한 명만 연락이 닿지 않아도 투자가 멈추고, 그 사이 조건이 바뀌거나 무산될 수 있습니다.",
      low: "선이 낮으면 웬만한 투자를 다 같이 정하게 됩니다. 대신 작은 라운드마다 절차를 밟아야 해서 속도가 느려집니다.",
      high: "선이 높으면 대표 주주가 혼자 결정할 수 있는 범위가 넓어집니다. 그만큼 지분이 희석되는 결정을 다른 주주가 뒤늦게 알게 됩니다.",
    },
  },
  {
    id: "deadlock",
    ...
    info: {
      ...
      advisory: "협의 기간이 지나도 합의가 안 되면 대표 주주가 정합니다. 이 절차로도 풀리지 않는 갈등이 생기면 외부 법률 전문가의 도움을 받아 해결하세요. 이 계약서에서 실제로 다툼이 생기는 영역은 여기와 퇴사 시 지분 회수, 두 곳입니다.",
  ...
  },
  {
    ...
    info: {
      what: "지분을 처음에 전부 주지 않고, 일한 기간만큼 나누어 확정하는 구조입니다. 확정은 매월 균등하게 쌓이며, 국내 스타트업에서 가장 흔한 방식입니다. 제5조 ②항의 액면가 매수권도 비슷한 효과를 내지만 전부 아니면 전무라, 단계가 없습니다.",
  ...
  },
  {
    ...
    info: {
      ...
      advisory: "사유에 따라 회수 가격을 다르게 하고 싶다면 여기서 정하지 마세요. 어떤 경우를 \"본인 잘못\"으로 볼지, 주식 가치를 얼마로 볼지는 전문가와 함께 정해야 합니다.",
      low: "액면가로 일괄하면 산정이 명확해 다툼이 적습니다. 오래 기여하고 떠나는 사람도 회사 가치 상승분을 받지 못하는 구조가 됩니다.",
      high: "사유를 나누려면 어떤 경우가 \"본인 잘못\"인지 조문에 하나하나 적어야 합니다. 회사 사정으로 내보내는 경우까지 여기에 걸리면 해고가 정당했는지를 두고 다투게 되어, 주식 회수와는 다른 성격의 분쟁으로 번집니다. 그래서 이 합의안에서는 나누지 않고, 필요하면 전문가와 정하도록 남겨둡니다.",
    },
  },
  {
    id: "lockup",
    ...
    info: {
      ...
      advisory: "이 조항은 넣어 두기를 권합니다. 다만 실제로 주식을 팔아 버린 거래 자체를 되돌리기는 어렵고, 위약벌이 함께 있어야 실효가 생깁니다.",
  ...
  },
  {
    ...
    preview: {
      ...
      blocks: [
        { kind: "para", text: "① 제6조 및 제7조에도 불구하고, 회사 발행주식총수의 {1}% 이상을 보유한 주주(수인의 주주가 합산하여 위 비율에 이르는 경우를 포함하며, 이하 '매도요구주주'라고 한다)가 보유 주식 전부를 제3자에게 양도하고자 하는 경우, 매도요구주주는 다른 주주들에게 동일한 조건으로 그 보유 주식 전부 또는 일부를 함께 양도할 것을 요구할 수 있는 권리(이하 \"공동매도요구권\"이라고 함)를 가진다." },
    ...
    info: {
      ...
      advisory: "몇 %로 걸지는 지분 구조에 따라 달라집니다. 셋이 비슷하게 나눠 가진 팀과 대표가 대부분을 가진 팀은 같은 숫자가 전혀 다르게 작동합니다. 도입하기로 했다면 관련 전문가에게 꼭 상담을 받으세요.",
  ...
  },
  {
    ...
    info: {
      ...
      advisory: "이 권리는 이 합의안에 원래 들어 있는 조항입니다. 넣지 않기로 하면 있던 조항을 빼는 것이 됩니다. 어떤 식으로 도입할지는 실무적인 판단이 필요하니, 관련 전문가에게 꼭 상담을 받으세요.",
  ...
  },
  {
    ...
    preview: {
      ...
      blocks: [
        ...
        // ⑤항은 "처분행위를 한 경우"를 전제로 처분 금액에 비율을 곱한다. 드래그얼롱 위반은
        // 파는 것이 아니라 안 파는 것이라 처분 금액이 없어 ⑤항이 작동하지 않는다.
        // 1차 자문의 "제재가 안 써져 있으면 계약은 선언에 불과"를 제7의 2조에도 적용한다.
        { kind: "para", text: "⑥ 이 계약의 어느 당사자가 제7의 2조 제3항의 양도 의무 또는 협력 의무를 이행하지 아니하는 경우 해당 당사자는 매도요구주주에게 손해배상과 별도로 위약벌로서 각 {0}원을 지급하여야 한다." },
      ],
    },
    info: {
      ...
      advisory: "이 조항은 넣어 두기를 권합니다. 계약서의 다른 조항들은 어겼을 때 물어야 할 것이 정해져 있어야 실제로 지켜집니다.",
