...
export const CONTRACT_QUESTIONS: ContractQuestion[] = [
  ...
  },
  {
    ...
    desc: "이 합의안은 사유를 가리지 않고 액면가로 정리합니다. 사유에 따라 다르게 하고 싶다면 전문가와 함께 정해야 합니다.",
    // 2차 변호사 자문: 차등 구조를 여기서 만들지 않는다.
    // "이유를 막론한 퇴사가 있는 경우에는 무조건 액면가로 정리되는 것으로 일단
    //  계약서를 만들어 두는 게 안전해요."
    // "액면가로 하고, 만약에 차등을 두고 싶은 경우에는 별도 자문을 받아라라고
    //  넘기는 게 제일 안전할 것으로 보여요."
    // 귀책/일반을 조문에서 가르면 부당해고 다툼으로 번지기 때문이다.
    template: {
      type: "choice",
      options: [
        { id: "par", label: "사유를 가리지 않고 액면가", desc: "이 합의안의 기본 문안입니다. 계산이 단순하고 가격을 두고 다툴 일이 없습니다." },
        { id: "consult", label: "사유에 따라 다르게 하고 싶어요", desc: "여기서는 정하지 않습니다. 어떤 경우를 어떻게 나눌지는 노무사·변호사와, 가치 산정은 회계사와 함께 정하세요. 합의안에는 액면가로 남습니다." },
      ],
    },
    preview: {
      ...
      blocks: [
        ...
        // 조문은 액면가로 고정하고, 사용자의 선택은 합의 메모로 남긴다.
        // 차등을 원한다고 답해도 합의안 문안은 바꾸지 않는다 — 전문가와 정할 사항이다.
        { kind: "para", text: "[합의 메모] 회수 가격 기준 — {0}" },
      ],
    },
    info: {
