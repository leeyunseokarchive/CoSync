...
**Interfaces:**
- Consumes: Task 1(에뮬레이터 배선), Task 2(시드 데이터 — `demo-team-a`, `demo-team-b`, 로그인 `owner@demo.local` / `demopass123!`).
- Produces: 1280x800 PNG 스크린샷 4장 (Task 5에서 `ScreenshotFrame` 배경으로 사용).

에뮬레이터 계정(`owner@demo.local`)은 이번 시드 스크립트가 만든 테스트 픽스처이며 실제 사용자 자격증명이 아니므로, 에이전트가 브라우저에서 직접 로그인해 진행한다.

- [ ] **Step 1: 에뮬레이터 + dev 서버 기동**
...
브라우저 도구로 `http://localhost:3000/login` 이동 → 이메일 `owner@demo.local`, 비밀번호 `demopass123!` 입력 → 로그인 버튼 클릭 → `/workspace`로 리다이렉트 확인.

- [ ] **Step 3: 온보딩 진단 화면 캡처**

`http://localhost:3000/onboarding/diagnosis` 이동 → 뷰포트 1280x800 → 스크린샷 → `public/screenshots/02-diagnosis.png` 저장.

- [ ] **Step 4: 갭 리포트 화면 캡처**

`http://localhost:3000/gap-report?teamId=demo-team-a` 이동 → 히트맵/카테고리 섹션이 보이도록 스크롤 → 스크린샷 → `public/screenshots/03-gap-report.png` 저장.

- [ ] **Step 5: 합의 세션 화면 캡처**

`http://localhost:3000/consensus?teamId=demo-team-a` 이동 → resolved(초록)/voting(진행중) 항목이 함께 보이는 스크롤 위치 → 스크린샷 → `public/screenshots/04-consensus.png` 저장.

- [ ] **Step 6: 합의서 문서 화면 캡처**

`http://localhost:3000/agreement/document?teamId=demo-team-b` 이동 → 조항 목록이 보이는 위치 → 스크린샷 → `public/screenshots/05-agreement-document.png` 저장.
...
Expected: 4개 파일 모두 `PNG image data, 1280 x 800` (또는 캡처 시 사용한 실제 해상도)로 출력, 파일 크기 0바이트 아님.

- [ ] **Step 8: 서버 정리**

에뮬레이터 및 dev 서버 백그라운드 프로세스 종료 (`kill` 또는 터미널 종료).
